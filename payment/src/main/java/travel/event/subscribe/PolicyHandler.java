...
public class PolicyHandler {
    ...
    @Retryable(value = CustomException.class, maxAttempts = 3, backoff = @Backoff(delay = 1000))
    @StreamListener(value = KafkaProcessor.INPUT, condition = "headers['type']=='FlightReservationCancelRequested'")
    public void wheneverflightReservationCancelRequested(@Payload FlightReservationCancelRequested flightReservationCancelRequested) {
    ...
    public void paymentStateRecover(CustomException e, Object paymentInfo) {
        if (paymentInfo instanceof FlightReservationRequested) {
            logger.error("\n예약 정보를 토대로 결제 정보 저장에 실패했습니다.\n");
        } else if (paymentInfo instanceof FlightReservationCancelRequested) {
